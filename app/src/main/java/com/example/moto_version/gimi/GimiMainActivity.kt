...
class GimiMainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val navView = findViewById<NavigationView>(R.id.gimi_nav_view)

        // Configurar listener para el botón flotante
        val fabMenu = findViewById<FloatingActionButton>(R.id.gimi_fab_menu)
            ...
            .addSnapshotListener { snapshots, error ->
                ...
                snapshots.documents.forEach { doc ->
                    ...
                    if (latitudProveedor != null && longitudProveedor != null && latitudCliente != null && longitudCliente != null) {
                        ...
                        puntosRecojoLista.add(PuntoPedidoCliente(id, ubicacionCliente, clienteNombre, proveedorNombre, pedidoCantidadCobrar, pedidoMetodoPago, fechaEntregaPedidoMotorizado, fechaRecojoPedidoMotorizado, thumbnailFotoRecojo, fechaAnulacionPedido, ubicacionProveedor, thumbnailFotoEntrega))
    ...
    private fun agregarMarcadores() {
        mMap?.let { map ->
            ...
            for (punto in puntosRecojoLista) {
                ...
                when {
                    // Pedido anulado -> Color rosa, ubicación del cliente
                    punto.fechaAnulacionPedido != null -> {
                        color = BitmapDescriptorFactory.HUE_ROSE
                        posicion = punto.ubicacionCliente
                        titulo = "Entrega: ${punto.clienteNombre}"
                    }
                    // Pedido entregado -> Color verde, ubicación del cliente
                    punto.fechaEntregaPedidoMotorizado != null -> {
                        color = BitmapDescriptorFactory.HUE_GREEN
                        posicion = punto.ubicacionCliente
                        titulo = "Entrega: ${punto.clienteNombre}"
                    }
                    // Pedido aún no recogido -> Color amarillo, ubicación del proveedor
                    punto.fechaRecojoPedidoMotorizado == null -> {
                        color = BitmapDescriptorFactory.HUE_YELLOW
                        posicion = punto.ubicacionProveedor
                        titulo = "Recojo: ${punto.proveedorNombre}"
                    }
                    // Pedido recogido pero no entregado -> Color azul, ubicación del cliente
                    else -> {
